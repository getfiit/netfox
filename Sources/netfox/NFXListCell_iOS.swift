...
class NFXListCell: UITableViewCell {
    ...
    override func layoutSubviews() {
        ...
        methodLabel.frame = CGRect(x: statusView.frame.maxX + padding, y: URLLabel.frame.maxY - 2, width: 50, height: frame.height - URLLabel.frame.maxY - 2)
    ...
    func configForObject(_ obj: NFXHTTPModel) {
        ...
        if obj.requestURL?.contains("graphql") == true, obj.requestType == "application/json" {
            configGraphQLForObject(obj)
        } else {
    ...
    func configGraphQLForObject(_ obj: NFXHTTPModel) {
        let requestBody: String = obj.getRequestBody() as String
        
        let stopCharacter = CharacterSet(charactersIn: "( ")
        
        if let mutationRange: Range<String.Index> = requestBody.range(of: "mutation "), let nameRange: Range<String.Index> = requestBody.rangeOfCharacter(from: stopCharacter, range: mutationRange.upperBound..<requestBody.endIndex) {
            setMethod("mutation")
            setType(String(requestBody[mutationRange.upperBound..<nameRange.lowerBound]))
        }
        
        else if let queryRange: Range<String.Index> = requestBody.range(of: "query "), let nameRange: Range<String.Index> = requestBody.rangeOfCharacter(from: stopCharacter, range: queryRange.upperBound..<requestBody.endIndex) {
            setMethod("query")
            setType(String(requestBody[queryRange.upperBound..<nameRange.lowerBound]))
        }
